...
def naiveSoftmaxLossAndGradient(
    ...
    # conditional prob dist P(O = o | C = c)
    y_hat = softmax(np.matmul(outsideVectors, centerWordVec))

    # loss is J_naive_softmax(v_c, o, U) = − log P(O = o|C = c). 
    loss = -np.log(y_hat[outsideWordIdx])

    # backprop 
    # The true empirical distribution y is a one-hot vector with a 1 for the true outside word o,
    # and 0 everywhere else
    y = np.zeros_like(y_hat)
    y[outsideWordIdx] = 1

    gradCenterVec = np.inner(np.transpose(outsideVectors), y_hat - y)
...
def negSamplingLossAndGradient(
    ...
    for i in range(1, K+1):
        ...
        loss += -np.log(y_k_hat)
        y_k_hat -= 1
        gradCenterVec -= y_k_hat*outsideVectors[w_k]
        gradOutsideVecs[w_k] -= y_k_hat*centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    center_word_vecs = centerWordVectors[center_word_idx]

    for outside_word in outsideWords:
        outside_word_idx = word2Ind[outside_word]
        step_loss, grad_center, grad_outside = word2vecLossAndGradient(center_word_vecs, outside_word_idx, outsideVectors, dataset)
        loss += step_loss
